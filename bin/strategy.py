...
def create_strategy_table():
    with DBWorker() as (_, cursor):
        cursor.execute(
            ...
            "`create_time` INT NOT NULL COMMENT '创建策略',"
            "`update_time` INT NOT NULL COMMENT '上传日期',"
...
def migrate_strategy():
    with DBWorker() as (_, cursor):
        ...
        # 保存
        cursor.executemany(
            "INSERT INTO work_strategy (id,create_time,update_time,author_id,content,variety_en,contract,"
            "direction,hands,open_price,close_price,profit,note)"
            "VALUES (%(id)s,%(create_time)s,%(update_time)s,%(author_id)s,%(content)s,%(variety_en)s,%(contract)s,"
            "%(direction)s,%(hands)s,%(open_price)s,%(close_price)s,%(profit)s,%(note)s);",
            strategy
        )
